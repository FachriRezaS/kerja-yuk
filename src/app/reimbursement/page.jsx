...
import { Search, Filter, Receipt, MoreHorizontal, BarChart3 } from "lucide-react";
...
export default function ReimbursementPage() {
    ...
    const allClaims = [
        ...
        { id: "CLM-002", employeeName: "Andi Susanto", type: "Client Entertainment", date: "02 Jul 2026", amount: "Rp 250.000", status: "Pending", description: "Lunch with prospective client" },
        ...
        { id: "CLM-004", employeeName: "Citra Lestari", type: "Medical Allowance", date: "10 Jul 2026", amount: "Rp 850.000", status: "Pending", description: "Dental checkup" },
        { id: "CLM-005", employeeName: "Eko Pratama", type: "Travel & Transport", date: "05 Jul 2026", amount: "Rp 2.500.000", status: "Rejected", description: "Flight to Bali for conference" },
    ];
    ...
    return (
        <DashboardLayout title="Reimbursements (Manager)">
            <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-6">
                ...
                {!isManager ? (
                    <div className="flex flex-col items-center justify-center h-64 text-center">
                        ...
                        <p className="text-gray-500 dark:text-gray-400 mt-2 max-w-md">You do not have permission to view the Reimbursements Management dashboard.</p>
                    </div>
                ) : (
                    <>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
                            <Card className="bg-gradient-to-br from-red-600 to-red-700 border-0 text-white shadow-lg shadow-red-500/20">
                                <CardContent className="p-6">
                                    <p className="text-red-100 font-bold uppercase tracking-wider text-xs mb-2">Total Claimed (YTD)</p>
                                    <h3 className="text-3xl font-black">Rp 45.2M</h3>
                                    <p className="text-sm text-red-200 mt-2 font-medium">124 approved claims this year</p>
                                </CardContent>
                            </Card>
                            <Card className="md:col-span-2">
                                <CardContent className="p-6 flex items-center gap-6">
                                    <div className="w-16 h-16 rounded-2xl bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 flex items-center justify-center shrink-0">
                                        <BarChart3 size={32} />
                                    </div>
                                    <div className="flex-1">
                                        <h3 className="font-bold text-gray-900 dark:text-white text-lg">Monthly Claim History</h3>
                                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 mb-3">Claim volume is up 12% compared to last month.</p>
                                        <div className="flex items-end gap-2 h-10">
                                            {[40, 60, 30, 80, 50, 90, 70].map((h, i) => (
                                                <div key={i} className="flex-1 bg-red-100 dark:bg-red-900/30 rounded-t-sm hover:bg-red-500 dark:hover:bg-red-500 transition-colors cursor-pointer" style={{ height: `${h}%` }}></div>
                                            ))}
                                        </div>
                                    </div>
                                </CardContent>
                                        ...
                                        {filtered.map((claim) => (
                                            <tr key={claim.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors cursor-pointer" onClick={() => setSelectedClaim(claim)}>
                                                ...
                                                <td className="px-6 py-4">
                                                    <p className="font-semibold text-gray-900 dark:text-white">{claim.employeeName}</p>
                                                </td>
